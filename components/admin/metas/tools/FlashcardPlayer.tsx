...
const FlashcardPlayer: React.FC<FlashcardPlayerProps> = ({ cards, onClose, title = "Flashcard" }) => {
  ...
  // States matching gamified player
  const [isStarted, setIsStarted] = useState(false);
  const [isFinished, setIsFinished] = useState(false);
  const [isShuffleEnabled, setIsShuffleEnabled] = useState(false);
  const [playableCards, setPlayableCards] = useState<Flashcard[]>([]);
  const [score, setScore] = useState(0);
  const [totalAcertos, setTotalAcertos] = useState(0);
  const [totalErros, setTotalErros] = useState(0);

  // Reset state when opening or when cards change
  useEffect(() => {
    setCurrentIndex(0);
    setIsFlipped(false);
    setIsStarted(false);
  ...
  }, [isStarted, isFinished, isFlipped, handleNext, handlePrev, handleFlip, handleAcertei, handleErrei, onClose]);

  if (!cards || cards.length === 0) return null;

  const currentCard = playableCards[currentIndex] || cards[0];
    ...
    <div className="fixed inset-0 z-[200] bg-black text-white font-sans flex flex-col overflow-hidden animate-in fade-in duration-300 select-none">
      ...
      <div className="relative z-10 flex flex-col w-full bg-zinc-950/80 border-b border-zinc-800 backdrop-blur-md">
        ...
        <div className="w-full h-1 bg-zinc-900">
          <div 
            ...
            style={{ width: `${isStarted && !isFinished ? progress : 0}%` }}
          />
        </div>
        ...
        <div className="flex items-center justify-between px-6 py-4">
          <div className="flex items-center gap-4">
             ...
             <div>
                <h2 className="text-sm md:text-lg font-black uppercase tracking-tighter text-white leading-none mb-1">
                   {title}
                </h2>
                <div className="flex items-center gap-2 text-[10px] font-mono text-zinc-500 uppercase tracking-widest">
                   <Layers size={10} />
                   <span>Flashcard Deck (Modo Teste)</span>
                </div>
             </div>
          </div>
      ...
      {!isStarted && (
        <div className="flex-1 flex flex-col items-center justify-center p-6 z-10 animate-in fade-in zoom-in-95 duration-350">
           <div className="w-16 h-16 rounded-2xl border border-purple-500/30 bg-purple-500/10 flex items-center justify-center mb-6 text-purple-400 shadow-[0_0_20px_rgba(168,85,247,0.2)]">
              <Layers size={32} />
           </div>
           ...
           <p className="text-zinc-500 text-xs md:text-sm font-bold uppercase tracking-widest mb-8">
              Contém {cards.length} {cards.length === 1 ? 'flashcard' : 'flashcards'}
           </p>
           ...
           <div className="w-full max-w-md bg-zinc-900/40 border border-zinc-800/80 p-6 rounded-2xl mb-8 flex flex-col gap-4">
              <div className="flex items-center justify-between">
                 ...
                 <button 
                    ...
                    className={`relative w-12 h-6 rounded-full transition-colors duration-300 flex items-center ${isShuffleEnabled ? 'bg-purple-600' : 'bg-zinc-800'}`}
                 >
                    <div 
                       className={`w-5 h-5 bg-white rounded-full shadow-md transition-transform duration-300 ${isShuffleEnabled ? 'translate-x-[26px]' : 'translate-x-0.5'}`}
                    />
                 </button>
              </div>
           </div>
           ...
           <button
              ...
              className="px-8 py-4 bg-purple-600 hover:bg-purple-500 text-white font-black text-xs md:text-sm uppercase tracking-widest rounded-2xl transition-all hover:scale-105 active:scale-95 shadow-lg shadow-purple-600/25 flex items-center gap-3"
           >
        ...
        <div className="relative flex-1 flex items-center justify-between px-4 md:px-12 perspective-container">
          ...
          >
             <div className={`card-inner ${isFlipped ? 'flipped' : ''}`}>
                ...
                {/* --- FRONT FACE (QUESTION) --- */}
                <div className="card-face card-front bg-zinc-900 border border-zinc-850 rounded-3xl shadow-[0_0_50px_rgba(0,0,0,0.5)] flex items-center justify-center p-8 md:p-16 relative overflow-hidden group-hover:border-purple-500/50 transition-colors">
                    {/* Decorative Elements */}
                    <div className="absolute top-6 left-6 w-2 h-2 bg-purple-500 rounded-full shadow-[0_0_10px_#a855f7]"></div>
                    <div className="absolute top-6 right-6 text-[10px] font-mono text-zinc-655 uppercase tracking-widest">Pergunta</div>
                ...
                <div className="card-face card-back bg-zinc-950 border border-emerald-500/30 rounded-3xl shadow-[0_0_50px_rgba(16,185,129,0.1)] flex flex-col p-8 md:p-12 relative overflow-hidden">
                    ...
                    <div className="absolute top-6 left-6 text-[10px] font-mono text-emerald-500 uppercase tracking-widest flex items-center gap-2">
                        <div className="w-1.5 h-1.5 bg-emerald-500 rounded-full"></div> Resposta (Estágio de Validação)
                    </div>
                    ...
                    <div className="mt-4 mb-2 flex flex-col sm:flex-row items-center justify-center gap-4 shrink-0" onClick={e => e.stopPropagation()}>
                        <button 
                          onClick={handleErrei}
                          className="flex items-center justify-center gap-2 px-8 py-3.5 bg-red-655 hover:bg-red-550 text-white font-black text-xs uppercase tracking-widest rounded-2xl transition-all hover:scale-105 active:scale-95 shadow-lg shadow-red-900/20 border border-red-500/35 w-full sm:w-auto"
                        >
                          <ThumbsDown size={14} />
                          Errei
                        ...
                        <button 
                          ...
                          className="flex items-center justify-center gap-2 px-8 py-3.5 bg-emerald-655 hover:bg-emerald-555 text-white font-black text-xs uppercase tracking-widest rounded-2xl transition-all hover:scale-105 active:scale-95 shadow-lg shadow-emerald-900/20 border border-emerald-500/35 w-full sm:w-auto"
                        >
                          <ThumbsUp size={14} />
                          Acertei
      ...
      {isStarted && isFinished && (
        <div className="flex-1 flex flex-col items-center justify-center p-6 z-10 animate-in fade-in zoom-in-95 duration-300">
           <div className="w-20 h-20 rounded-full bg-purple-500/10 border border-purple-500/30 flex items-center justify-center mb-6 text-purple-400 shadow-[0_0_30px_rgba(168,85,247,0.2)]">
              <Trophy size={40} />
           </div>
           ...
           <div className="w-full max-w-sm bg-zinc-900/50 border border-zinc-800 p-6 rounded-3xl mb-8 space-y-4 shadow-2xl">
              <div className="flex items-center justify-between border-b border-zinc-800 pb-3">
                 <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider">Total de Cards Testados</span>
           ...
           <div className="flex flex-col sm:flex-row gap-4 w-full max-w-sm">
              ...
              <button
                 onClick={onClose}
                 className="flex-grow py-4 bg-purple-600 hover:bg-purple-500 text-white font-black text-xs uppercase tracking-widest rounded-2xl transition-all shadow-lg shadow-purple-600/20 flex items-center justify-center gap-2"
              >
                 Pronto
              </button>
           </div>
      ...
      {/* --- FOOTER HINTS --- */}
      {isStarted && !isFinished && (
        <div className="relative z-10 bg-zinc-950/90 border-t border-zinc-805 py-4 flex flex-col md:flex-row items-center justify-center gap-8 backdrop-blur-md text-[10px] font-bold text-zinc-500 uppercase tracking-widest">

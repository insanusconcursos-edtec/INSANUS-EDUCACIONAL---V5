...
import { ShieldAlert, Send, Clock, CheckCircle2, XCircle, AlertTriangle, Scale } from 'lucide-react';
...
export const ExamResult: React.FC<ExamResultProps> = ({ exam, attemptData, onBack }) => {
    ...
    // Recursos states
    const [studentResources, setStudentResources] = useState<SimulatedResource[]>([]);
    const [allExamResources, setAllExamResources] = useState<SimulatedResource[]>([]);
    const [loadingResources, setLoadingResources] = useState(false);
    
    // Form states
    const [resQuestionNumber, setResQuestionNumber] = useState<number>(1);
    const [resType, setResType] = useState<'alterar_gabarito' | 'anular_questao'>('alterar_gabarito');
    const [resJustification, setResJustification] = useState('');
    const [resNewAlternative, setResNewAlternative] = useState('A');
    const [submittingResource, setSubmittingResource] = useState(false);

    const loadResourcesData = async () => {
        if (!currentUser || !exam.id) return;
        setLoadingResources(true);
        try {
            const [studentRes, allRes] = await Promise.all([
                getResourcesForStudent(currentUser.uid, exam.id),
                getResourcesForExam(exam.id)
            ]);
            setStudentResources(studentRes);
            setAllExamResources(allRes);
        } catch (error) {
        ...
        <div className="fixed inset-0 z-[100000] w-screen h-screen bg-[#0a0a0a] flex flex-col animate-in fade-in slide-in-from-right-8 duration-500 overflow-hidden">
                ...
                <div className="max-w-7xl mx-auto px-6">
                    ...
                    <div className="hidden md:flex items-center gap-8 overflow-x-auto">
                        ...
                        <button
                            ...
                            className={`py-4 px-2 text-xs font-bold uppercase tracking-widest transition-all border-b-2 flex items-center gap-2 ${activeTab === 'RECURSOS' ? 'border-red-600 text-white' : 'border-transparent text-gray-500 hover:text-gray-300'}`}
                        >
                            <Scale className="w-4 h-4" />
                            Recursos & Retificações
                        </button>
                    </div>
                        ...
                        <div className="relative w-full">
                            ...
                            >
                                <div className="flex items-center gap-3">
                                    ...
                                    {activeTab === 'RECURSOS' && <Scale size={16} className="text-blue-500" />}
                                    <span>
                                        {activeTab === 'PERFORMANCE' ? 'Meu Desempenho' : 
                                         activeTab === 'AUTODIAGNOSIS' ? 'Autodiagnóstico' : 
                                         activeTab === 'RANKING' ? 'Ranking Geral' : 'Recursos & Retificações'}
                                    </span>
                                </div>
                            ...
                            {isDropdownOpen && (
                                <div className="absolute top-full left-0 right-0 mt-2 bg-[#1A1A1A] border border-zinc-800 rounded-xl shadow-2xl z-[210] overflow-hidden">
                                    ...
                                    <button 
                                        ...
                                        className={`w-full flex items-center gap-3 px-4 py-4 text-[10px] font-black uppercase tracking-widest transition-colors border-b border-zinc-800/50 ${activeTab === 'RANKING' ? 'text-emerald-500 bg-emerald-500/5' : 'text-zinc-500 hover:bg-zinc-800'}`}
                                    >
                                        <Trophy size={16} />
                                        Ranking Geral
                                    </button>
                                    <button 
                                        onClick={() => { setActiveTab('RECURSOS'); setIsDropdownOpen(false); }}
                                        className={`w-full flex items-center gap-3 px-4 py-4 text-[10px] font-black uppercase tracking-widest transition-colors ${activeTab === 'RECURSOS' ? 'text-blue-500 bg-blue-500/5' : 'text-zinc-500 hover:bg-zinc-800'}`}
                                    >
                                        <Scale size={16} />
                                        Recursos & Retificações
                                    </button>
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-8 pt-10 md:pt-20">
                <div className="max-w-7xl mx-auto">
                    ...
                    {/* --- ABA 4: RECURSOS E RETIFICAÇÕES --- */}
                    {activeTab === 'RECURSOS' && (
                        <div className="space-y-8 animate-in fade-in duration-300">
                            {/* Retificações Oficiais */}
                            <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6">
                                <h3 className="text-sm font-black text-white flex items-center gap-2 uppercase tracking-widest mb-4">
                                    ...
                                    Retificações Oficiais do Simulado
                                </h3>
                                <p className="text-[11px] text-zinc-500 uppercase tracking-wider mb-6">
                                ...
                                {allExamResources.some(r => r.status === 'accepted' || r.status === 'accepted_summary') || exam.questions?.some(q => q.isAnnulled) ? (
                                    ...
                                    <div className="p-6 bg-zinc-950 rounded-lg border border-zinc-800/50 text-center text-zinc-500 text-xs font-bold uppercase tracking-wider">
                                        Nenhuma retificação oficial registrada para este simulado até o momento.
                                    </div>
                                )}
                            </div>

                            {/* Enviar Recurso Form & Meus Recursos */}
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                                {/* Formulario */}
                                <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6 flex flex-col">
                                    <h3 className="text-sm font-black text-white flex items-center gap-2 uppercase tracking-widest mb-2">
                                        <Send size={18} className="text-brand-red" />
                                        Interpor Novo Recurso
                                    </h3>
                                    <p className="text-[10px] text-zinc-500 uppercase tracking-wider mb-6">
                                        Caso discorde de alguma questão ou gabarito, envie a sua justificativa fundamentada para análise.
                                    </p>

                                    <form onSubmit={async (e) => {
                                        e.preventDefault();
                                        if (!resJustification.trim()) {
                                        ...
                                        try {
                                            await submitResource({
                                                ...
                                                userName: userData?.name || currentUser?.displayName || 'Aluno',
                                                userEmail: currentUser?.email || '',
                                                questionNumber: resQuestionNumber,
                                                type: resType,
                                                justification: resJustification,
                                                newAlternative: resType === 'alterar_gabarito' ? resNewAlternative : undefined
                                            });
                                        ...
                                        <div>
                                            <label className="block text-[10px] font-black uppercase text-zinc-500 tracking-wider mb-2">Número da Questão</label>
                                            <select 
                                                value={resQuestionNumber}
                                                onChange={(e) => setResQuestionNumber(Number(e.target.value))}
                                        ...
                                        <div>
                                            <label className="block text-[10px] font-black uppercase text-zinc-500 tracking-wider mb-2">O que deseja solicitar?</label>
                                            <select 
                                                value={resType}
                                                onChange={(e) => setResType(e.target.value as any)}
                                                className="w-full bg-zinc-950 text-white text-xs font-bold p-3 rounded-lg border border-zinc-800 focus:border-brand-red focus:outline-none uppercase"
                                            >
                                                <option value="alterar_gabarito">Alterar Gabarito</option>
                                                <option value="anular_questao">Anular Questão</option>
                                            </select>
                                        </div>

                                        {resType === 'alterar_gabarito' && (
                                            <div>
                                                <label className="block text-[10px] font-black uppercase text-zinc-500 tracking-wider mb-2">Alternativa que considera Correta</label>
                                                <select 
                                                    value={resNewAlternative}
                                                    onChange={(e) => setResNewAlternative(e.target.value)}
                                                    className="w-full bg-zinc-950 text-white text-xs font-bold p-3 rounded-lg border border-zinc-800 focus:border-brand-red focus:outline-none uppercase"
                                                >
                                                    {['A', 'B', 'C', 'D', 'E'].map(opt => (
                                        ...
                                        <div className="flex-1 flex flex-col">
                                            <label className="block text-[10px] font-black uppercase text-zinc-500 tracking-wider mb-2">Justificativa / Fundamentação</label>
                                            <textarea 
                                                placeholder="Escreva detalhadamente sua justificativa..."
                                                value={resJustification}
                                                onChange={(e) => setResJustification(e.target.value)}
                                                className="w-full min-h-[120px] flex-1 bg-zinc-950 text-white text-xs font-medium p-3 rounded-lg border border-zinc-800 focus:border-brand-red focus:outline-none resize-none"
                                                required
                                            />
                                        </div>
                                        ...
                                        <button
                                            ...
                                            className="w-full py-3 bg-brand-red hover:bg-red-600 text-white text-xs font-black uppercase tracking-widest rounded-lg transition-colors flex items-center justify-center gap-2"
                                        >
                                            {submittingResource ? 'Enviando...' : 'Enviar Recurso'}
                                        </button>
                                    </form>
                                </div>
                                ...
                                <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6 flex flex-col">
                                    <h3 className="text-sm font-black text-white flex items-center gap-2 uppercase tracking-widest mb-2">
                                        ...
                                        Meus Recursos Enviados
                                    </h3>
                                    <p className="text-[10px] text-zinc-500 uppercase tracking-wider mb-6">
                                        Acompanhe o andamento dos seus recursos interpostos neste simulado.
                                    </p>

                                    {loadingResources ? (
                                        <div className="flex-1 flex items-center justify-center text-zinc-500 uppercase font-bold text-xs tracking-widest">
                                            Carregando recursos...
                                        </div>
                                    ) : studentResources.length > 0 ? (
                                        <div className="space-y-4 max-h-[450px] overflow-y-auto custom-scrollbar flex-1 pr-1">
                                            {studentResources.map((res) => (
                                                <div key={res.id} className="bg-zinc-950/50 border border-zinc-800 rounded-xl p-4 space-y-3">
                                                    <div className="flex justify-between items-start gap-2">
                                                        <div>
                                                            <span className="text-[10px] font-mono text-zinc-500 uppercase">Questão {res.questionNumber}</span>
                                                            <h4 className="text-white font-black text-xs uppercase mt-0.5">
                                                                {res.type === 'anular_questao' ? 'Anulação' : `Alteração para ${res.newAlternative}`}
                                                            </h4>
                                                        </div>
                                                    ...
                                                    <p className="text-[11px] text-zinc-400 italic font-medium leading-relaxed bg-zinc-900/30 p-2.5 rounded-lg border border-zinc-800/30">
                                                        "{res.justification}"
                                                    </p>
                                                    {res.adminResponse && (
                                                        <div className="bg-zinc-900 border-l-2 border-brand-red p-3 rounded-r-lg space-y-1">
                                                            <span className="text-[8px] font-black uppercase text-zinc-500 tracking-wider">Resposta do Administrador:</span>
                                        ...
                                        <div className="flex-1 flex flex-col items-center justify-center p-8 bg-zinc-950 rounded-lg border border-zinc-800/50 text-center opacity-60">
                                            <Scale size={28} className="text-zinc-600 mb-2" />
                                            <p className="text-[10px] text-zinc-500 font-black uppercase tracking-widest">Nenhum recurso interposto</p>
                                        </div>
                                    )}
                                </div>
                            </div>

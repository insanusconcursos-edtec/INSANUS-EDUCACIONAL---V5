    ...
    <div className="h-[calc(100vh-100px)] flex flex-col animate-in fade-in duration-500">
      ...
      {activeTab === 'CHAT' && planId && (
          <div className="flex-1 overflow-hidden flex flex-col gap-4">
              <div className="shrink-0 flex items-center justify-between bg-zinc-900/60 p-4 rounded-xl border border-zinc-800/80">
                  <div className="flex flex-col">
                      <span className="text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-0.5">Visibilidade do Chat com Alunos</span>
                      <p className="text-xs text-zinc-400">
                          {plan?.isChatDisabled 
                            ? "O chat de mentoria está temporariamente desativado e invisível para os alunos." 
                            : "O chat de mentoria está ativo e visível para os alunos."}
                      </p>
                  </div>
                  <button
                      onClick={() => handleUpdatePlan({ isChatDisabled: !plan?.isChatDisabled })}
                      className={`px-4 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all border ${
                          plan?.isChatDisabled 
                          ? 'bg-green-600/10 hover:bg-green-600/20 text-green-500 border-green-500/30' 
                          : 'bg-red-600/10 hover:bg-red-600/20 text-red-500 border-red-500/30'
                      }`}
                  >
                      {plan?.isChatDisabled ? "Ativar Chat" : "Desativar Chat Temporariamente"}
                  </button>
              </div>
              <div className="flex-1 overflow-hidden">

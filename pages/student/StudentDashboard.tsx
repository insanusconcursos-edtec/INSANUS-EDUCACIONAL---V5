...
const StudentDashboard: React.FC = () => {
      ...
      try {
          ...
          if (fullPlan.linkedSimuladoClassId) {
              try {
                  ...
                  const sortedExams = exams.sort((a, b) => 0); 
  ...
  if (currentTab === 'call') {
      ...
      if (fullPlanData?.isChatDisabled) {
          return (
              <div className="relative w-full h-[calc(100vh-140px)] bg-zinc-950 flex flex-col items-center justify-center p-6 animate-in fade-in duration-500">
                  <div className="text-center max-w-md p-8 bg-zinc-900 border border-zinc-800 rounded-2xl shadow-xl">
                      <div className="w-16 h-16 bg-red-600/10 text-red-500 rounded-full flex items-center justify-center mx-auto mb-6">
                          <MessageSquare size={32} />
                      </div>
                      <h3 className="text-xl font-black text-white tracking-tight mb-2">Chat Desativado Temporariamente</h3>
                      <p className="text-sm text-zinc-400 leading-relaxed">
                          O chat com o mentor está temporariamente desativado para este plano. Por favor, tente novamente mais tarde ou entre em contato com o suporte.
                      </p>
                  </div>
              </div>
          );

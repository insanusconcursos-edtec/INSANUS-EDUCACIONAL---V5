    ...
    <div className="flex items-center">
      ...
      {hasChildren && !node.collapsed && (
        <div className="flex flex-col justify-center ml-10 relative animate-in fade-in slide-in-from-left-2 duration-300">
          ...
          <div 
            className="absolute left-[-40px] top-1/2 h-[2.5px] w-10 -translate-y-1/2 pointer-events-none transition-all duration-300" 
            style={{ backgroundColor: node.color || '#52525b' }}
          />
          ...
          {node.children!.map((child, index) => {
            ...
            return (
              <div key={child.id} className="relative flex items-center py-2">
                ...
                {/* SVG para linhas onduladas/curvadas (Curvas de Bezier dinâmicas) */}
                <svg 
                  className="absolute left-0 top-0 w-8 h-full pointer-events-none overflow-visible"
                  viewBox="0 0 32 100" 
                  preserveAspectRatio="none"
                >
                  {/* Linha do tronco vertical - Apenas para nós intermediários, evitando vazamentos e linhas que extrapolam */}
                  {!isFirst && !isLast && (
                    <>
                      <line 
                        x1="0" y1="0" x2="0" y2="50" 
                        stroke={node.color || "#52525b"} strokeWidth="2" 
                      />
                      <line 
                  ...
                  {/* Curva ondulada para o nó filho */}
                  <path 
                    d={
                      index < middleIndex 
                        ? "M 0,100 C 0,50 16,50 32,50" 
                        : index > middleIndex 
                        ? "M 0,0 C 0,50 16,50 32,50" 
                        : "M 0,50 L 32,50"
                    } 
                    fill="none" 
                    stroke={child.color || "#52525b"} 
                    strokeWidth="2.5" 
                    className="transition-all duration-300"
                  />
                </svg>
                
                {/* Espaçador para o SVG da esquerda */}
                <div className="w-8 h-px pointer-events-none" />

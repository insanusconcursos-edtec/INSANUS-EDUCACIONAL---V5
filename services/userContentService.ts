...
export const getUserContent = async (userId: string, planId: string, metaId: string, type: UserContentType): Promise<UserContentItem[]> => {
  ...
  const items = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as UserContentItem));
  
  // Sort in memory (descending by createdAt) to avoid requiring a composite index
  return items.sort((a, b) => {
    const valA = a.createdAt?.seconds || (a.createdAt instanceof Date ? a.createdAt.getTime() / 1000 : 0);
    const valB = b.createdAt?.seconds || (b.createdAt instanceof Date ? b.createdAt.getTime() / 1000 : 0);
    return valB - valA;
  });
};

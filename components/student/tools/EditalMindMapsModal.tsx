...
export const EditalMindMapsModal: React.FC<EditalMindMapsModalProps> = ({
  ...
  const handleRestartMindMap = async () => {
    if (!currentUser || !selectedItem) return;
    setIsLoading(true);
    try {
      // Delete old document from database
      await deleteUserContent(currentUser.uid, planId, selectedItem.id);

      // Immediately create a new blank mind map document
      await createUserContent(currentUser.uid, planId, editalNodeId, 'MAP', selectedItem.title);

      // Fetch items again
      const data = await getUserContent(currentUser.uid, planId, editalNodeId, 'MAP');
      setItems(data);

      // Select the newly created mind map
      if (data.length > 0) {
            ...
            <div className="h-full flex flex-col items-center justify-center text-center p-8">
             ...
             <div className="h-full flex flex-col animate-in fade-in duration-300">
                ...
                <MindMapFullscreen 
                    ...
                    readOnly={false}
                    onRestart={handleRestartMindMap}
                />
             </div>

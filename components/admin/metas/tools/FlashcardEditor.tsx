...
const FlashcardEditor: React.FC<FlashcardEditorProps> = ({ cards, onChange }) => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const generatedCards = await generateFlashcardsFromDocuments(selectedFiles, quantity, customPrompt);
        ...
        {!isGenerating && !hasCards && (
            <div className="p-6 flex flex-col gap-6 animate-in fade-in duration-500">
                ...
                <div className="space-y-3 bg-zinc-900/40 p-4 rounded-xl border border-zinc-800/80">
                    <h4 className="text-[10px] font-black text-pink-500 uppercase tracking-widest flex items-center gap-1.5">
                        <BrainCircuit size={12} /> Configurações da Geração por IA
                    </h4>
                    <div className="grid grid-cols-3 gap-3">
                        <div className="col-span-1 flex flex-col gap-1">
                            <label className="text-[9px] font-black text-zinc-400 uppercase tracking-wider">Qtde de Cards</label>
                            <input
                                type="number"
                                min={1}
                                max={100}
                                placeholder="Auto (IA decide)"
                                value={quantity || ''}
                                onChange={(e) => {
                                    const val = e.target.value;
                                    setQuantity(val ? parseInt(val, 10) : null);
                                }}
                                className="w-full bg-zinc-950 border border-zinc-800 focus:border-pink-500/50 focus:ring-1 focus:ring-pink-500/20 text-xs px-2.5 py-1.5 rounded-lg text-white placeholder-zinc-600 outline-none transition-all"
                            />
                        </div>
                        <div className="col-span-2 flex flex-col gap-1">
                            <label className="text-[9px] font-black text-zinc-400 uppercase tracking-wider">Diretrizes / Limitações (Opcional)</label>
                            <input
                                type="text"
                                placeholder="Ex: Gerar do art. 20 ao 40..."
                                value={customPrompt}

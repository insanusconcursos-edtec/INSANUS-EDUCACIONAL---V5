...
export function TeachingEnvironment({ classId, masterClassId }: TeachingEnvironmentProps) {
  // --- ESTADOS DE MÓDULOS ---
  ...
  const loadModules = async () => {
    ...
    try {
      ...
      let data = await courseService.getModules(classId);
      
      // Lógica de Herança: Se a turma filha não tem módulos, tenta buscar da turma mãe
      if (data.length === 0 && masterClassId) {
        const masterData = await courseService.getModules(masterClassId);
      ...
      <div className="flex items-center justify-between border-b border-gray-800 pb-4 shrink-0">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              {isInherited && (
                <span className="bg-blue-500/10 text-blue-400 text-[10px] font-black px-2 py-0.5 rounded border border-blue-500/20 uppercase">Herdado da Turma Mãe</span>
              )}
            </div>

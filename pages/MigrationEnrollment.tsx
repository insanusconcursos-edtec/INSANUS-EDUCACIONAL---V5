...
export default function MigrationEnrollment() {
  ...
  const handleCheckIn = (e: React.FormEvent) => {
    e.preventDefault();
    const trimmedEmail = checkInEmail.trim().toLowerCase();
    if (!trimmedEmail) {
      toast.error('Por favor, digite um e-mail válido.');
      return;
    }

    const authorizedList = linkData?.authorizedEmails || [];
    
    if (authorizedList.length === 0) {
      toast.error('Este link não possui e-mails pré-autorizados cadastrados.');
      return;
    }

    const matched = authorizedList.find((item: any) => item.email.toLowerCase() === trimmedEmail);

    if (!matched) {
      toast.error('Este e-mail não está autorizado para este link de migração.');
      return;
    }

    if (matched.status === 'completed') {
      toast.error('Este e-mail já realizou o processo de migração e não pode ser reutilizado.');
      return;
    }
    ...
    setFormData(prev => ({
      ...prev,
      email: trimmedEmail,
      confirmEmail: trimmedEmail
    }));
    toast.success('E-mail validado! Continue o preenchimento de seus dados.');
  };
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 3.5. Atualizar status na lista de e-mails autorizados do link de migração
      if (linkData?.id && linkData?.authorizedEmails && Array.isArray(linkData.authorizedEmails)) {
        const linkRef = doc(db, 'MigrationLinks', linkData.id);
        const updatedEmails = linkData.authorizedEmails.map((item: any) => {
          if (item.email.toLowerCase() === formData.email.toLowerCase()) {
            return {
              ...item,
              status: 'completed',
        ...
        await updateDoc(linkRef, { authorizedEmails: updatedEmails });
      }
  ...
  const requiresCheckIn = linkData?.authorizedEmails && Array.isArray(linkData.authorizedEmails) && linkData.authorizedEmails.length > 0;

  if (requiresCheckIn && !validatedEmail) {
    return (
      <div className="min-h-screen bg-zinc-950 flex items-center justify-center p-4 py-12">
        <div className="max-w-md w-full bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden shadow-2xl animate-in fade-in slide-in-from-bottom-4 duration-500">
          <div className="p-8 bg-zinc-950/50 border-b border-zinc-800 text-center space-y-2">
            ...
            <p className="text-zinc-500 text-xs mt-1">Este link de migração exige que seu e-mail esteja previamente autorizado.</p>
          </div>
          ...
          <form onSubmit={handleCheckIn} className="p-8 space-y-6">
            <div className="space-y-4">
              <div className="p-4 bg-zinc-950 border border-zinc-800/80 rounded-xl">
                <p className="text-zinc-400 text-xs leading-relaxed">
                  Digite abaixo o e-mail previamente autorizado para este link de migração para prosseguir com o seu cadastro/login.
                </p>
              </div>
              ...
              <div className="relative">
                ...
                <input 
                  ...
                  className="w-full bg-zinc-950 border border-zinc-800 rounded-xl py-4 pl-12 pr-4 text-white focus:outline-none focus:border-red-600 transition-all text-sm"
                />
              </div>
            </div>
            ...
            <button
              type="submit"
              className="w-full py-4 bg-red-600 hover:bg-red-500 text-white font-black uppercase tracking-widest rounded-xl transition-all shadow-lg shadow-red-900/20 flex items-center justify-center gap-2 text-xs"
            >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-in slide-in-from-left-4 duration-300">
              ...
              <div className="relative">
                ...
                <input 
                  type="email"
                  required
                  readOnly={!!requiresCheckIn}
                  placeholder="E-mail"
                  value={formData.email}
                  onChange={(e) => !requiresCheckIn && setFormData({ ...formData, email: e.target.value })}
                  className={`w-full border rounded-xl py-4 pl-12 pr-4 transition-all ${
                    requiresCheckIn 
                      ? 'bg-zinc-950/50 border-zinc-800/50 text-zinc-400 cursor-not-allowed' 
                      : 'bg-zinc-950 border-zinc-800 text-white focus:outline-none focus:border-red-600'
                  }`}
                />
                {requiresCheckIn && (
                  <div className="absolute right-4 top-1/2 -translate-y-1/2 text-emerald-500 flex items-center gap-1 text-[10px] font-black uppercase tracking-wider">
              ...
              <div className="relative">
                ...
                <input 
                  ...
                  onChange={(e) => !requiresCheckIn && setFormData({ ...formData, confirmEmail: e.target.value })}
                  className={`w-full border rounded-xl py-4 pl-12 pr-4 transition-all ${
                    requiresCheckIn 
                      ? 'bg-zinc-950/50 border-zinc-800/50 text-zinc-400 cursor-not-allowed' 
                      : 'bg-zinc-950 border-zinc-800 text-white focus:outline-none focus:border-red-600'
                ...
                />
                {!requiresCheckIn && formData.confirmEmail && formData.email !== formData.confirmEmail && (

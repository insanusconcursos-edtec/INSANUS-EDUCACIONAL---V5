    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-left-4 duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="col-span-2 border-t border-zinc-800 pt-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {data.linkedCourseId && (
              <div>
                <input 
                  type="text"
                  placeholder="Nome Principal do Curso (Ex: CURSO ONLINE)"
                  value={data.linkedCourseTabLabel || ''}
                  onChange={(e) => {
                    const val = e.target.value;
                    const currentTabs = data.linkedCourseSharedTabs || {};
                    onChange({ 
                      linkedCourseTabLabel: val,
                      linkedCourseSharedTabs: {
                        ...currentTabs,
                        modules: {
                          enabled: currentTabs.modules?.enabled !== false,
                          label: val
                        }
                      }
                    });
          ...
          {data.linkedCourseId && (
            <div className="mt-4 bg-zinc-900/40 border border-zinc-800 rounded-xl p-4 space-y-4">
              <span className="text-xs font-bold text-zinc-400 uppercase tracking-wider block">
                Escolha quais abas compartilhar e defina os nomes de exibição na área do aluno:
              </span>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="space-y-2 p-3 bg-zinc-900/60 rounded-lg border border-zinc-800">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input 
                      type="checkbox"
                      checked={data.linkedCourseSharedTabs?.modules?.enabled !== false}
                      onChange={(e) => {
                        const currentTabs = data.linkedCourseSharedTabs || {};
                        onChange({
                          linkedCourseSharedTabs: {
                            ...
                            modules: {
                              ...
                              label: currentTabs.modules?.label || data.linkedCourseTabLabel || 'MÓDULOS DO CURSO'
                            }
                          }
                        });
                      }}
                      className="rounded border-zinc-700 bg-zinc-800 text-brand-red focus:ring-brand-red h-4 w-4"
                    />
                    <span className="text-xs font-bold text-white uppercase">Módulos do Curso</span>
                  </label>
                  <input 
                    type="text"
                    placeholder="Nome da Aba"
                    value={data.linkedCourseSharedTabs?.modules?.label ?? data.linkedCourseTabLabel ?? 'MÓDULOS DO CURSO'}
                    onChange={(e) => {
                      const currentTabs = data.linkedCourseSharedTabs || {};
                      const val = e.target.value;
                      onChange({
                    ...
                    className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-2.5 py-1.5 text-xs text-white focus:outline-none focus:border-brand-red transition-colors placeholder-zinc-600 uppercase font-bold"
                  />
                </div>
                ...
                <div className="space-y-2 p-3 bg-zinc-900/60 rounded-lg border border-zinc-800">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input 
                      type="checkbox"
                      checked={!!data.linkedCourseSharedTabs?.edital?.enabled}
                      onChange={(e) => {
                        const currentTabs = data.linkedCourseSharedTabs || {};
                        onChange({
                          linkedCourseSharedTabs: {
                            ...
                            edital: {
                              enabled: e.target.checked,
                              label: currentTabs.edital?.label || 'EDITAL VERTICALIZADO'
                            }
                          }
                        });
                      }}
                      className="rounded border-zinc-700 bg-zinc-800 text-brand-red focus:ring-brand-red h-4 w-4"
                    />
                    <span className="text-xs font-bold text-white uppercase">Edital Verticalizado</span>
                  </label>
                  <input 
                    type="text"
                    placeholder="Nome da Aba"
                    value={data.linkedCourseSharedTabs?.edital?.label || 'EDITAL VERTICALIZADO'}
                    onChange={(e) => {
                      const currentTabs = data.linkedCourseSharedTabs || {};
                      onChange({
                        linkedCourseSharedTabs: {
                          ...currentTabs,
                          edital: {
                            enabled: !!currentTabs.edital?.enabled,
                ...
                <div className="space-y-2 p-3 bg-zinc-900/60 rounded-lg border border-zinc-800">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input 
                      type="checkbox"
                      checked={!!data.linkedCourseSharedTabs?.live?.enabled}
                      onChange={(e) => {
                        const currentTabs = data.linkedCourseSharedTabs || {};
                        onChange({
                          linkedCourseSharedTabs: {
                            ...
                            live: {
                              enabled: e.target.checked,
                              label: currentTabs.live?.label || 'EVENTOS AO VIVO'
                            }
                          }
                        });
                      }}
                      className="rounded border-zinc-700 bg-zinc-800 text-brand-red focus:ring-brand-red h-4 w-4"
                    />
                    <span className="text-xs font-bold text-white uppercase">Eventos ao Vivo</span>
                  </label>
                  <input 
                    type="text"
                    placeholder="Nome da Aba"
                    value={data.linkedCourseSharedTabs?.live?.label || 'EVENTOS AO VIVO'}
                    onChange={(e) => {
                      const currentTabs = data.linkedCourseSharedTabs || {};
                      onChange({
                        linkedCourseSharedTabs: {
                          ...currentTabs,
                          live: {
                            enabled: !!currentTabs.live?.enabled,
                            label: e.target.value
                          }
                        }
                      });
          ...
          <p className="text-[10px] text-zinc-500 mt-1 uppercase">
            Ao vincular, as abas selecionadas serão ativadas na área do aluno desta turma com o conteúdo do curso selecionado.
          </p>
        </div>
      </div>
    </div>

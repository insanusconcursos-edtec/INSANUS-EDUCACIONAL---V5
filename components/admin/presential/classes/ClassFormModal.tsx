  ...
  const validateStep = (step: number): boolean => {
    switch (step) {
      ...
      case 3: // Schedule
        ...
        const hasRegularWeekends = formData.regularWeekendConfigs && formData.regularWeekendConfigs.some(config => config.shifts && config.shifts.length > 0);
        if ((!formData.daysOfWeek || formData.daysOfWeek.length === 0) && !hasRegularWeekends) {
          alert('Selecione pelo menos um dia da semana ou configure aulas regulares aos finais de semana.');
          return false;
        }
